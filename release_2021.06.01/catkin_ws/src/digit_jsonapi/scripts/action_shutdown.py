...
'''
    ...
    @file   action_shutdown.py: Send 'action-shutdown' command to Digit with JSON API
    @author Grant Gibson (Biped Robotics Lab, University of Michigan)
    @brief  Send 'action-shutdown' command to Digit with JSON API
 '''
...
async def main(ip_address):
    ...
    async with agility.JsonApi(address=ip_address) as api:
        ...
        # current position
        await api.wait_action('action-shutdown')
    
if __name__ == "__main__":
    # Identify experiment mode
    exp_mode = rospy.get_param('action_shutdown_node/exp_mode')
    if exp_mode == 'sim':
