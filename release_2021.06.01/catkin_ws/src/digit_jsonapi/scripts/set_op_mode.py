...
'''
    ...
    @brief  Set Digit's operation mode with JSON API
 '''
...
async def main(ip_address,mode):
    ...
    async with agility.JsonApi(address=ip_address) as api:
        ...
        # Set operation mode
        await api.wait_action(["action-set-operation-mode",{"mode": mode}])
    
if __name__ == "__main__":
    
    # Identify experiment mode
    exp_mode = rospy.get_param('action_stand_node/exp_mode')
    op_mode = rospy.get_param('set_op_mode_node/op_mode')
    if exp_mode == 'sim':
    ...
    asyncio.get_event_loop().run_until_complete(main(ip_address,op_mode))