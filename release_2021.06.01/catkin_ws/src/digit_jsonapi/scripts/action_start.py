...
'''
    ...
    @file   action_start.py
    @author Grant Gibson (Biped Robotics Lab, University of Michigan)
    @brief  Send 'action-start' command to Digit with JSON API
 '''
...
async def main(ip_address):
    ...
    async with agility.JsonApi(address=ip_address) as api:
        ...
        # Send disabled set operation mode
        await api.wait_action(["action-set-operation-mode",{"mode": "disabled"}])

        # Send action start command which will make robot stand
        await api.wait_action('action-start')
            
if __name__ == "__main__":
    
    # Identify experiment mode
    exp_mode = rospy.get_param('action_start_node/exp_mode')
    if exp_mode == 'sim':
